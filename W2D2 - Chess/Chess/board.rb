...
class Board
  ...
  def populate
    ...
    @grid[1].map!.with_index do |empt, idx|
      Pawn.new(:team1, @board, [1, idx])
    end

    @grid[6].map!.with_index do |empt, idx|
      Pawn.new(:team2, @board, [6, idx])
    end

    @grid[7] = [Rook.new(:team2, @board, [7, 0]), Knight.new(:team2, @board, [7, 2]),
      Bishop.new(:team2, @board, [7, 2]), King.new(:team2, @board, [7, 3]),
      Queen.new(:team2, @board, [7, 4]), Bishop.new(:team2, @board, [7, 5]),
      Knight.new(:team2, @board, [7, 6]), Rook.new(:team2, @board, [7, 7])]
